...
2. **Google OAuth app**: [Google Cloud Console](https://console.cloud.google.com/)
   ...
   - `https://<your-netlify-domain>/api/auth/callback/google` (production)
...
3. **Microsoft Entra app**: [Entra admin center](https://entra.microsoft.com/)
   ...
   - `https://<your-netlify-domain>/api/auth/callback/microsoft-entra-id`
...
5. Put all of the above into `packages/admin/.env.local` (git-ignored —
   ...
   full list of placeholder keys) **and** as Netlify site environment
   variables (Site settings → Environment variables) — the build needs them
